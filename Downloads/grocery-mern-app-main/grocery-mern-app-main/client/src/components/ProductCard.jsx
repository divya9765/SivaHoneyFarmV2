...
const ProductCard = ({ product }) => {
  ...
  const categoryFallback = (cat) => {
    const key = (cat || "").toLowerCase();
    if (key === "hair oils") return assets.bottles_image;
    if (key === "makeup products") return assets.bottom_banner_image_sm;
    if (key === "soap") return assets.bottom_banner_image;
    if (key === "pure honey") return assets.main_banner_bg_sm;
    if (key === "organic products") return assets.organic_vegitable_image;
        ...
        <div className="group cursor-pointer flex items-center justify-center px-2">
          <div className="w-28 h-28 md:w-36 md:h-36 flex items-center justify-center">
            <img
              ...
              alt={product.name}
              onError={(e) => {
                e.currentTarget.src = categoryFallback(product.category);
              }}
            />
          </div>
        </div>
