...
const ProductList = () => {
  ...
  const editProduct = async (product) => {
    try {
      ...
      const offerStr = window.prompt("Offer Price", String(product.offerPrice));
      if (offerStr === null) return;
      const description = window.prompt("Description", product.description || "");
      if (description === null) return;

      const price = Number(priceStr);
      const offerPrice = Number(offerStr);
      if (Number.isNaN(price) || Number.isNaN(offerPrice)) {
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-3 py-1 text-white bg-indigo-500 hover:bg-indigo-600 rounded"
                      >
                        Edit
                      </button>
                      <button
